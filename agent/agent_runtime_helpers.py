...
def _gemini_auth_client(agent, client_kwargs: dict, httpx_verify, *, reason: str, shared: bool):
    """Gemini Auth (per-user-quota) transport client, else None.

    Separate from ``_gemini_native_client``: same Gemini request body, but the OAuth bearer token
    replaces the API key and the endpoint is the ``:generateContentPerUserQuota`` variant, so the
    transport must be the Antigravity-aware facade rather than the plain native client.
    """
    from agent.gemini_auth_adapter import GEMINI_AUTH_BASE_URL, GeminiAuthClient
    if getattr(agent, "provider", "") != "gemini-auth":
        return None
    ...
    _ra().logger.info(
        "Gemini Auth client created (%s, shared=%s) %s", reason, shared, agent._client_log_context()
    )
    return client
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if agent.provider in _GEMINI_NATIVE_PROVIDER_NAMES:
        ...
        if client is not None:
            return client
    client = _gemini_auth_client(agent, client_kwargs, httpx_verify, reason=reason, shared=shared)
    if client is not None:
        return client
