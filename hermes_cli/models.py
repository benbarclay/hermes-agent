...
def _gemini_auth_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    """Curated picker list for Gemini Auth (per-user-quota).
    ...
    try:
        profile = get_provider_profile("gemini-auth")
    except Exception:
        return None
    if profile is not None and profile.fallback_models:
        return list(profile.fallback_models)
    return list(_PROVIDER_MODELS.get("gemini-auth", [])) or None
...
_PROVIDER_CATALOG_FETCHERS: dict[str, Any] = {
    ...
    "gemini-auth": _gemini_auth_catalog}
