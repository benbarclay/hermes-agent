...
_OAUTH_ONLY_HIDDEN_PROVIDERS = {"gemini-auth"}
...
_PROVIDER_ALIASES = {
    ...
    "gemini-auth": "gemini-auth", "google-gemini-auth": "gemini-auth",
    "gemini-auth-oauth": "gemini-auth"}
...
_HIDDEN_OAUTH_PROVIDERS = {"gemini-auth"}
...
def _gemini_auth_login(args) -> dict:
    """Run the Gemini Auth PKCE loopback login and return its token bundle for the pool entry.

    The full OAuth handshake (NAS-brokered exchange, PKCE verifier, loopback callback) lives in
    ``hermes_cli.gemini_auth``; this only adapts its result to the ``_OAuthAddSpec`` shape.
    """
    from hermes_cli.gemini_auth import GEMINI_AUTH_INFERENCE_BASE_URL, _login_gemini_auth

    _login_gemini_auth(args, None)
    state = auth_mod._load_provider_state(auth_mod._load_auth_store(), "gemini-auth") or {}
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    ...
    "gemini-auth": _OAuthAddSpec(
        login=_gemini_auth_login,
        token=lambda creds: creds["access_token"],
...
def auth_add_command(args) -> None:
    ...
    if requested_type == "api-key":
        ...
        oauth_default = (provider in _OAUTH_CAPABLE_PROVIDERS
