...
def _model_flow_gemini_auth(config, current_model="", args=None):
    ...
    if not get_gemini_auth_status().get("logged_in"):
        ...
        if not _run_login(_login_gemini_auth, mock_args, None):
            return
    ...
    base_url = creds.get("base_url") or GEMINI_AUTH_INFERENCE_BASE_URL
    from hermes_cli.models import provider_model_ids
    model_ids = provider_model_ids("gemini-auth")
    selected = _prompt_model_selection(model_ids, current_model, confirm_provider="gemini-auth",
                                       confirm_base_url=base_url)
    _activate_provider_model(selected, "gemini-auth", base_url,
                             f"\u2713 Using Gemini Auth model: {selected}")
